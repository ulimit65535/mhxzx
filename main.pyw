...
class AppUI:
    def __init__(self):
        ...
        lf2_0 = ttk.LabelFrame(lf2, text="无情刷子")
        ...
        self.btn_team_task = ttk.Button(lf2_3, text="开始任务", state=DISABLED, command=self.test)
        ...
        size = '%dx%d+%d+%d' % (width, height, 1080, 250)
        root.geometry(size)
        ...
        while True:
            if self.process or self.subprocess:
                ...
                while True:
                    ...
                    # 恢复按键状态
                    self.btn_brush.state(['!pressed', '!disabled'])
    ...
    def brush(self):
        self.running = self.brush_task.get()
        self.btn_brush.state(['pressed', 'disabled'])
